...
class MqttServerSecureConnection extends MqttServerConnection<SecureSocket> {
  ...
  Future<MqttClientConnectionStatus?> connect(
    ...
    try {
      ...
      if (applied) {
        MqttLogger.log('MqttSecureConnection::connect - socket options applied');
      }
      client = socket;
  ...
  Future<MqttClientConnectionStatus?> connectAuto(
    ...
    try {
      final socket = await SecureSocket.connect(server, port, onBadCertificate: onBadCertificate, context: context);

      MqttLogger.log('MqttSecureConnection::connectAuto - securing socket');
      // Socket options
      final applied = _applySocketOptions(socket, socketOptions);
      if (applied) {
        MqttLogger.log('MqttSecureConnection::connectAuto - socket options applied');
      }
      client = socket;
